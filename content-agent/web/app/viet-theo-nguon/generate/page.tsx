...
import type { SourceConfig, SourceItem, SourceStreamResult } from '@/lib/viet-theo-nguon/types';
...
interface DbArticlePayload {
  ...
  targetLength: number;
  aiProvider: string;
  brandConfig?: SourceConfig['brandConfig'];
  outline?: {
    stage?: string;
    structure?: SourceConfig['structure'];
...
function parseSourceStructure(contentType: string): SourceConfig['structure'] {
  const raw = contentType.startsWith('viet_theo_nguon:') ? contentType.slice('viet_theo_nguon:'.length) : '';
  const allowed: SourceConfig['structure'][] = [
    'auto',
    'inverted_pyramid',
    'storytelling',
    'qa',
  ...
  return allowed.includes(raw as SourceConfig['structure']) ? (raw as SourceConfig['structure']) : 'auto';
}
...
function buildOutlinePayload(config: SourceConfig, sources: SourceItem[], outlineText: string, aiCheckStorageKey?: string) {
  return {
    stage: 'generate',
...
export default function VietTheoNguonGeneratePage() {
  ...
  const aiCheckStorageKey = useMemo(() => (articleId ? `aicheck:source:${articleId}` : undefined), [articleId]);
  ...
  useEffect(() => {
    document.title = 'Generate Viet Theo Nguon - Content Agent';
    void bootstrap();
  ...
  async function bootstrap() {
    const storedConfig = sessionStorage.getItem(STORAGE_KEYS.config);
    const storedArticleId = sessionStorage.getItem(STORAGE_KEYS.articleId);
    const storedRunId = runIdParam || sessionStorage.getItem(STORAGE_KEYS.runId) || '';

    if (!storedRunId) {
      router.replace('/viet-theo-nguon');
      return;
    }

    if (storedConfig && storedArticleId && !runIdParam) {
      try {
        const parsedConfig = JSON.parse(storedConfig) as SourceConfig;
    ...
    try {
      ...
      const nextConfig: SourceConfig = article.outline?.config ?? {
        ...
        customOutline: '',
        structure: parseSourceStructure(article.contentType),
        tone: 'formal',
      ...
      const nextOutlineText = article.outline?.outline ?? '';

      setConfig(nextConfig);
      setArticleId(article.id);
      setSources(nextSources);
      setOutlineText(nextOutlineText);
      sessionStorage.setItem(STORAGE_KEYS.config, JSON.stringify(nextConfig));
      sessionStorage.setItem(STORAGE_KEYS.articleId, article.id);
      sessionStorage.setItem(STORAGE_KEYS.runId, article.runId);
      sessionStorage.setItem(STORAGE_KEYS.sources, JSON.stringify(nextSources));
      sessionStorage.setItem(STORAGE_KEYS.outline, nextOutlineText);
      setEditTitle(article.selectedTitle || article.keyword);
      setEditMetaDescription(article.metaDescription || '');
      setSecondaryKeywords(article.secondaryKeywords || nextConfig.secondaryKeywords || []);
      ...
      const nextAiCheckStorageKey = article.id ? `aicheck:source:${article.id}` : undefined;
      writeSessionAICheckState(nextAiCheckStorageKey, article.outline?.aiCheck);
      if (article.runId && article.runId !== article.id) {
        writeSessionAICheckState(`aicheck:source:${article.runId}`, null);
      }

      if (!article.htmlContent?.trim()) {
        await startGeneration(nextConfig, nextSources, nextOutlineText, article.runId, article.id);
        return;
      }
      ...
      const nextOutline = buildOutlinePayload(nextConfig, nextSources, nextOutlineText, nextAiCheckStorageKey);
      persistedSignatureRef.current = buildPersistedSignature(
        ...
        article.secondaryKeywords || nextConfig.secondaryKeywords || [],
        nextOutline,
      );
      sessionStorage.setItem(STORAGE_KEYS.result, JSON.stringify({
        runId: article.runId,
  ...
  async function startGeneration(
    ...
    setError('');
    setStatus('streaming');
    setStatusMessage('AI dang viet bai theo nguon...');
    setEditorHtml('');
    ...
    setOutlineText(currentOutlineText);

    try {
      const response = await fetch('/api/viet-theo-nguon/stream', {
        method: 'POST',
      ...
      while (true) {
        ...
        for (const event of events) {
          ...
          if (payload.type === 'done' && payload.data) {
            ...
            setSources(nextResult.sources || currentSources);
            ...
            persistedSignatureRef.current = buildPersistedSignature(
              ...
              secondaryKeywords.length > 0 ? secondaryKeywords : currentConfig.secondaryKeywords,
              buildOutlinePayload(currentConfig, nextResult.sources || currentSources, currentOutlineText, `aicheck:source:${currentArticleId}`),
            );
            sessionStorage.setItem(STORAGE_KEYS.result, JSON.stringify(nextResult));
            sessionStorage.setItem(STORAGE_KEYS.sources, JSON.stringify(nextResult.sources || currentSources));
            setLoading(false);
          }
        }
      }
  ...
  function insertInternalLink(html: string) {
    setBanner({ tone: 'success', text: 'Da chen internal link vao bai.' });
    insertAppendHtml(`<p style="margin-top:1rem">Xem them: ${html}</p>`);
  }
  ...
  function fixKeywordInContent() {
    const kw = config?.keyword.trim();
    if (!kw || !editorHtml) return;
    setEditorHtml(`${editorHtml}<section><h2>Ghi chu them ve ${escapeHtml(kw)}</h2><p>Khi danh gia ${escapeHtml(kw)}, can xem xet nhu cau su dung thuc te, ngan sach, tieu chi chat luong va cac diem can so sanh de chon phuong an phu hop.</p></section>`);
  ...
  function fixMinWordCount() {
    const kw = config?.keyword.trim();
    if (!kw || !editorHtml) return;
    const sentence = `${kw} can duoc can nhac theo muc dich su dung, tieu chi so sanh, ngan sach va nhung diem thuc te de nguoi doc de lua chon hon.`;
  ...
  function fixMetaLength() {
    ...
    if (nextMeta.length < 120) nextMeta = `${nextMeta} Noi dung tap trung vao thong tin can biet, cach lua chon va cac diem nen kiem tra truoc khi quyet dinh.`;
  ...
  function fixSecondaryKeyword() {
    ...
    setEditorHtml(`${editorHtml}<section><h2>Lien quan den ${escapeHtml(missingKeyword)}</h2><p>${escapeHtml(missingKeyword)} la yeu to nen duoc can nhac cung chu de chinh de noi dung bao quat hon va huu ich hon cho nguoi doc.</p></section>`);
    setBanner({ tone: 'success', text: 'Da chen tu khoa phu vao noi dung.' });
  }
  ...
  function fixH2Count() {
    ...
    if (h2Count >= 2) return;
    setEditorHtml(`${editorHtml}<section><h2>Cach chon ${escapeHtml(config.keyword)}</h2><p>${escapeHtml(config.keyword)} nen duoc danh gia theo muc tieu, cach ap dung va nhung diem can so sanh de chon dung phuong an.</p></section>`);
  ...
  function fixFaqSection() {
    ...
    setEditorHtml(`${editorHtml}<section><h2>FAQ ve ${safeKeyword}</h2><div class="faq-item"><h3>${safeKeyword} phu hop voi ai?</h3><p>${safeKeyword} phu hop voi nguoi dang can thong tin ro rang de so sanh, lua chon hoac lap ke hoach truoc khi ap dung.</p></div></section>`);
  ...
  async function saveDraft(createVersion: boolean) {
    ...
    const outlinePayload = buildOutlinePayload(config, sources, outlineText, aiCheckStorageKey);
    ...
    <div className="flex h-full flex-col bg-gray-50">
      <div className="flex items-center justify-between gap-4 border-b border-gray-200 bg-white px-6 py-4">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-wide text-blue-600">Viet Theo Nguon</p>
        ...
        <main className="flex min-w-0 flex-1 flex-col bg-gray-50">
          ...
          <div className="min-h-0 flex-1 overflow-hidden p-5">
            <div className="h-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
              <div ref={editorShellRef} className="h-full" data-vtn-editor>
                <RichArticleEditor
                  html={editorHtml}
