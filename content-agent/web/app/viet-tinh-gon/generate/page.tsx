...
import { buildOutlineFallback } from '@/lib/tinh-gon/outline';
import { buildTinhGonContentType, buildTinhGonSnapshot, parseTinhGonSnapshot } from '@/lib/tinh-gon/persistence';
...
interface DbArticlePayload {
  ...
  targetLength: number;
  aiProvider: string;
  brandConfig?: TinhGonConfig['brandConfig'];
  outline: unknown;
...
function parseOutlineType(contentType: string): TinhGonOutlineType {
  const raw = contentType.startsWith('tinh_gon:') ? contentType.slice('tinh_gon:'.length) : '';
  const allowed: TinhGonOutlineType[] = [
    'review_product',
    'how_to_choose',
    'compare',
    'faq',
  ...
  return allowed.includes(raw as TinhGonOutlineType) ? (raw as TinhGonOutlineType) : 'review_product';
}
...
function buildResultSignature(
  result: TinhGonStreamResult | null,
  html: string,
  title: string,
...
export default function VietTinhGonGeneratePage() {
  ...
  const [result, setResult] = useState<TinhGonStreamResult | null>(null);
  ...
  useEffect(() => {
    document.title = 'Generate Viết Tinh Gọn - Content Agent';
    void bootstrap();
    return () => {
  ...
  useEffect(() => {
    if (!articleId || !result || !config || !outline || loading) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [articleId, config, loading, outline, result, resultSignature]);
  ...
  async function bootstrap() {
    ...
    const storedRunId = sessionStorage.getItem('tg_run_id');

    if (runIdParam || !storedConfig || !storedOutline || !storedArticleId || !storedRunId) {
    ...
    try {
      ...
      const nextOutline = JSON.parse(storedOutline) as TinhGonOutlineData;
      ...
      if (storedResult) {
        const parsedResult = JSON.parse(storedResult) as TinhGonStreamResult;
      ...
      await startGeneration(nextConfig, nextOutline, nextRunId, nextArticleId);
    } catch {
      router.replace('/viet-tinh-gon');
    }
  }
  ...
  function applyLoadedResult(
    nextResult: TinhGonStreamResult,
    title: string,
    metaDescription: string,
    slug: string | null,
  ) {
  ...
  async function loadFromDatabase(targetRunId: string) {
    ...
    try {
      ...
      const snapshot = parseTinhGonSnapshot(article.outline);
      const fallbackConfig: TinhGonConfig = snapshot?.config || {
        keyword: article.keyword,
        outlineType: parseOutlineType(article.contentType),
        language: article.language,
        model: article.aiProvider,
        targetLength: article.targetLength,
        secondaryKeywords: article.secondaryKeywords || [],
        notes: article.userNotes || '',
        brandConfig: article.brandConfig,
      };
      const fallbackOutline = snapshot?.outline || buildOutlineFallback(fallbackConfig);

      const nextConfig = {
        ...fallbackConfig,
        secondaryKeywords: article.secondaryKeywords?.length ? article.secondaryKeywords : fallbackConfig.secondaryKeywords,
        notes: article.userNotes || fallbackConfig.notes,
      };
      const nextOutline = {
        ...fallbackOutline,
        selectedTitle: article.selectedTitle || fallbackOutline.selectedTitle,
        userNotes: article.userNotes || fallbackOutline.userNotes,
      };

      sessionStorage.setItem('tg_config', JSON.stringify(nextConfig));
      sessionStorage.setItem('tg_outline', JSON.stringify(nextOutline));
      sessionStorage.setItem('tg_run_id', article.runId);
      sessionStorage.setItem('tg_article_id', article.id);
      ...
      if (article.htmlContent) {
        const restoredResult: TinhGonStreamResult = {
          runId: article.runId,
        ...
        sessionStorage.setItem('tg_result', JSON.stringify(restoredResult));
        setLoading(false);
        void loadInternalLinks(nextConfig.keyword, restoredResult.html);
        return;
      }

      await startGeneration(nextConfig, nextOutline, article.runId, article.id);
    } catch (requestError) {
  ...
  async function startGeneration(
    currentConfig: TinhGonConfig,
    currentOutline: TinhGonOutlineData,
    currentRunId: string,
    ...
    setResult(null);
    setEditorHtml('');
    setEditTitle(currentOutline.selectedTitle);
      ...
      while (true) {
        ...
        for (const event of events) {
          ...
          if (payload.type === 'done' && payload.data) {
            ...
            sessionStorage.setItem('tg_result', JSON.stringify(payload.data));
            setLoading(false);
  ...
  function handleContentInput(nextHtmlArg?: string) {
    ...
    // Debounced re-check humanness sau khi user ng?ng g� 2.5 gi�y
    if (recheckTimerRef.current) clearTimeout(recheckTimerRef.current);
  ...
  function captureSelection() {
    ...
    setSelectionLabel(selectedText ? `�� ch?n ${selectedText.length} k� t? d? AI ch?nh.` : '');
  }
  ...
  function exportToWord() {
    ...
    anchor.href = url;
    anchor.download = `${editTitle.slice(0, 50).replace(/[^a-zA-Z0-9]/g, '-') || 'viet-tinh-gon'}.doc`;
    anchor.click();
  ...
  async function refreshMetrics(nextHtml?: string) {
    ...
    try {
      ...
      const updated: TinhGonStreamResult = {
        ...(result || {
          runId,
          title: editTitle,
      ...
      sessionStorage.setItem('tg_result', JSON.stringify(updated));
    } catch (requestError) {
  ...
  async function applyAiEdit(command: TinhGonEditCommand) {
    ...
    if (!selectedText) {
      setError('H�y b�i den do?n c?n ch?nh tr?c ti?p trong editor tru?c.');
      return;
    }
  ...
  async function saveDraft(createVersion: boolean) {
    if (!articleId || !config || !outline || !result) return false;
    const currentHtml = contentRef.current?.innerHTML ?? editorHtml;
    ...
    try {
        ...
        const response = await fetch(`/api/articles/${articleId}`, {
          ...
          body: JSON.stringify({
            ...
            outline: buildTinhGonSnapshot({
              ...
              config,
              outline,
              aiCheck: readSessionAICheckState(runId ? `aicheck:tinh-gon:${runId}` : undefined),
            }),
            htmlContent: currentHtml,
            metaDescription: editMetaDescription,
            slug: activeSlug || undefined,
      ...
      const nextResult: TinhGonStreamResult = {
        ...result,
        html: currentHtml,
        title: editTitle,
        metaDescription: editMetaDescription,
      ...
      sessionStorage.setItem('tg_result', JSON.stringify(nextResult));
      persistedSignatureRef.current = buildResultSignature(
        nextResult,
        currentHtml,
        editTitle,
  ...
  function handleDownload() {
    ...
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = `${editTitle.slice(0, 50).replace(/\s+/g, '-') || 'viet-tinh-gon'}.html`;
    anchor.click();
    URL.revokeObjectURL(url);
  }
    ...
    <div className="h-full flex flex-col bg-white overflow-hidden relative">
      <div className="border-b border-gray-200 bg-white flex-shrink-0">
        <div className="flex items-center gap-3 px-4 pt-2.5 pb-1.5">
          ...
          <button
            title="Làm mới"
            onClick={() => articleId && void startGeneration(config, outline, runId, articleId)}
        ...
        <div className="flex items-center gap-1 px-3 py-1.5 overflow-x-auto">
          <div className="flex items-center gap-1 mr-2 flex-shrink-0">
            ...
            <button
              ...
              onClick={() => router.push('/viet-tinh-gon/outline')}
            >
              ✍️ Rewrite <span className="text-gray-400">▾</span>
            </button>
          </div>
          ...
          <button
            title="Bài mới"
            onClick={() => {
              ['tg_config', 'tg_outline', 'tg_run_id', 'tg_result', 'tg_article_id', 'tg_outline_source', 'tg_outline_warning'].forEach((key) =>
      ...
      <div className="flex min-h-0 flex-1">
          ...
          <div className="min-h-0 flex-1 overflow-hidden p-5">
            <div className="h-full overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <p className="text-sm font-medium text-blue-700">AI đang viết bài tinh gọn...</p>
              </div>
        ...
        <div className="flex w-[420px] shrink-0 flex-col overflow-hidden border-l border-gray-200 bg-white">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs ${savedFlash ? 'text-green-600' : saving ? 'text-blue-600' : 'text-gray-400'}`}>
                    {savedFlash ? '�� luu' : saving ? '�ang luu...' : articleId ? 'DB linked' : ''}
                  </span>
                </div>
                <div className="text-xs text-gray-500 space-y-1">
                  ...
                  <p>Loại bài: <span className="text-gray-700">{config.outlineType}</span></p>
                </div>
                  ...
                  <div className="p-3">
                    ...
                    <div className="border border-gray-200 rounded-lg p-3 bg-white">
                      ...
                      <p className={`text-xs mt-1 line-clamp-3 leading-relaxed rounded px-2 py-1 transition-colors ${
                        ...
                        {editMetaDescription || 'Meta description s? hi?n th? ? d�y'}
                      </p>
                    </div>
                  </div>
                )}
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {!check.pass && index === 17 && (
                                  ...
                                  >
                                    {fixingTitleLength
                                      ? <><span className="inline-block w-3 h-3 border-2 border-purple-400 border-t-transparent rounded-full animate-spin" /> AI đang xử lý...</>
                                      : <>⚡ AI Fix — Chỉnh tiêu đề 50-70 ký tự</>
                                    }
                                  </button>
                                )}
                                ...
                                {!check.pass && index === 7 && (
                                  ...
                                  >
                                    {fixingSlugLength
                                      ? <><span className="inline-block w-3 h-3 border-2 border-purple-400 border-t-transparent rounded-full animate-spin" /> AI đang xử lý...</>
                                      : <>⚡ AI Fix — Rút gọn slug dưới 75 ký tự</>
                                    }
                                  </button>
                                )}
          ...
          <div className={sideTab === 'ai' ? 'flex-1 overflow-y-auto p-4 space-y-4' : 'hidden'}>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-4">
                <AICheckPanel
                  ...
                  storageKey={runId ? `aicheck:tinh-gon:${runId}` : undefined}
          ...
          <div className={sideTab === 'quality' ? 'flex-1 overflow-y-auto p-4 space-y-4' : 'hidden'}>
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-800">{(result?.keywordDensity ?? 0).toFixed(2)}%</span>
                </div>
                <KeywordDensityBar density={result?.keywordDensity ?? null} />
              </div>
            </div>
          ...
          <div className={sideTab === 'images' ? 'flex-1 overflow-y-auto p-4 flex flex-col items-center justify-center text-center' : 'hidden'}>
              <div className="text-4xl mb-3">🖼️</div>
              <p className="text-sm font-semibold text-gray-700 mb-1">Thư viện hình ảnh</p>
              <p className="text-xs text-gray-400 mb-4">
                M�n `tinh-gon` d� b�m layout `step4`, c�n module ?nh s? du?c n?i ti?p n?u b?n mu?n m�nh l�m ti?p.
              </p>
          </div>
        </div>
      </div>
            ...
            <div className="p-5">
              {imgModalTab === 'general' ? (
                ...
                <div>
                  ...
                  >
                    <span className="text-xs text-gray-500 mb-1">K�o th? ?nh v�o d�y</span>
                  ...
                  {imgUrl?.startsWith('data:') && (
                    <div className="mt-3">
                      ...
                      <p className="text-xs text-green-600 mt-1 text-center">? ?nh d� t?i l�n � nh?n &quot;Ch�n ?nh&quot; d? ch�n v�o b�i</p>
                    </div>
                  )}
                </div>
              )}
