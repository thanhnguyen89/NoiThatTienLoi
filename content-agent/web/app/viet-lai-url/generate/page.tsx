...
interface DbArticlePayload {
  ...
  targetLength: number;
  aiProvider: string;
  brandConfig?: UrlRewriteConfig['brandConfig'];
  outline?: {
...
function buildPublishReadiness(input: {
  ...
  const items: PublishSignal[] = [
    {
      key: 'title',
      label: 'Title có keyword và độ dài hợp lý',
    ...
    },
    {
      ...
      pass: wordCount >= 800,
      detail: `${wordCount.toLocaleString()}/800 từ`,
      priority: 'high',
    },
    {
      key: 'density',
...
export default function VietLaiUrlGeneratePage() {
  ...
  const [result, setResult] = useState<UrlRewriteResult | null>(null);
  ...
  const seoScore = useMemo(() => {
    ...
    return computeSeoChecks({
      ...
      secondaryKeywords: config.secondaryKeywords.split(',').map((item) => item.trim()).filter(Boolean),
      slug: activeSlug,
      minWordCount: 800,
    }).score;
  ...
  useEffect(() => {
    document.title = 'Generate Viết Lại URL - Content Agent';
    void bootstrap();
  ...
  useEffect(() => {
    ...
    previewRef.current.innerHTML = `
      <article class="prose prose-sm max-w-none">
        ${config.sourceTitle ? `<h1>${config.sourceTitle}</h1>` : ''}
        ${config.extractedHeadings
          ? `<pre style="white-space:pre-wrap;background:#f8fafc;border:1px solid #e5e7eb;border-radius:12px;padding:16px;font-family:ui-monospace, SFMono-Regular, Menlo, monospace;">${escapeHtml(config.extractedHeadings)}</pre>`
          : '<p><em>Chưa có heading nguồn.</em></p>'}
        ${config.extractedContent
          ? config.extractedContent
              ...
              .join('')
          : '<p><em>Chưa có nội dung nguồn.</em></p>'}
      </article>
    `;
  }, [config]);
  ...
  async function bootstrap() {
    ...
    try {
      const nextConfig = JSON.parse(storedConfig) as UrlRewriteConfig;
      setConfig(nextConfig);
      setArticleId(storedArticleId);
      setRunId(storedRunId);

      if (storedResult) {
        const parsedResult = JSON.parse(storedResult) as UrlRewriteResult;
      ...
      await startGeneration(nextConfig, storedRunId, storedArticleId);
    } catch {
      router.replace('/viet-lai-url');
    }
  }
  ...
  async function loadFromDatabase(targetRunId: string) {
    ...
    try {
      ...
      const nextConfig: UrlRewriteConfig = article.outline?.config ?? {
        ...DEFAULT_DB_CONFIG(article),
        brandConfig: article.brandConfig,
      };

      sessionStorage.setItem('vlu_config', JSON.stringify(nextConfig));
      sessionStorage.setItem('vlu_run_id', article.runId);
      sessionStorage.setItem('vlu_article_id', article.id);
      ...
      if (article.htmlContent) {
        const restoredResult: UrlRewriteResult = {
          runId: article.runId,
          ...
          imagesInjected: article.outline?.imagesInjected,
        };

        applyLoadedResult(restoredResult, article.selectedTitle, article.metaDescription || '', article.slug || null);
        sessionStorage.setItem('vlu_result', JSON.stringify(restoredResult));
        setLoading(false);
        return;
      }
  ...
  async function startGeneration(currentConfig: UrlRewriteConfig, currentRunId: string, currentArticleId: string) {
    setLoading(true);
    setError('');
    setStreamText('');
    setResult(null);
    setEditorHtml('');
    setEditTitle(currentConfig.keyword || currentConfig.sourceTitle || 'Viết lại URL');
      ...
      while (true) {
        ...
        for (const event of events) {
          ...
          if (payload.type === 'done' && payload.data) {
            ...
            sessionStorage.setItem('vlu_result', JSON.stringify(payload.data));
            setLoading(false);
          }
        }
      }
  ...
  async function saveDraft(createVersion: boolean) {
    ...
    try {
      ...
      const updated: UrlRewriteResult = {
        ...result,
        html: editorHtml,
        title: editTitle,
        metaDescription: editMetaDescription,
      ...
      setResult(updated);
      sessionStorage.setItem('vlu_result', JSON.stringify(updated));
    ...
    return (
      <div className="h-full flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-800">Đang viết lại từ URL...</h2>
          <p className="text-sm text-gray-500 mt-1 max-w-md">
            AI đang đọc heading, nội dung nguồn và tạo bài mới theo cấu trúc bạn đã chọn.
          </p>
          {streamText && (
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            type="button"
            onClick={() => router.push('/viet-lai-url')}
  ...
  return (
    <div className="h-full bg-gray-50 flex flex-col">
      <div className="px-6 py-4 bg-white border-b border-gray-200 flex items-center justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs text-blue-600 font-semibold uppercase tracking-wide">Viết Lại URL</p>
          <h1 className="text-xl font-bold text-gray-900 truncate">{editTitle || config.keyword || config.sourceTitle}</h1>
          <p className="text-xs text-gray-500 mt-1">
            {currentWordCount.toLocaleString()} từ · Density {currentKeywordDensity.toFixed(2)}% · Model {config.model}
            {result.imagesInjected ? ` · ${result.imagesInjected} ảnh Yandex` : ''}
          </p>
        </div>
        ...
        <div className="flex-1 min-w-0 grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div className="border-r border-gray-200 bg-white min-h-0 flex flex-col">
            <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
              <div>
                <h2 className="text-sm font-semibold text-gray-800">Nguồn URL</h2>
                <p className="text-xs text-gray-500 truncate">{config.sourceUrl}</p>
              </div>
            </div>
          ...
          <div className="bg-white min-h-0 flex flex-col">
            <div ref={editorShellRef} className="flex-1 min-h-0">
              <RichArticleEditor
                ...
                onNewArticle={() => router.push('/viet-lai-url')}
              />
            </div>
          </div>
        </div>
            ...
            {activeTab === 'seo' && (
              <div className="p-4 space-y-4">
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-2 text-xs text-gray-500">
                  ...
                  <p>Nguồn: <span className="text-gray-700 truncate">{config.sourceUrl}</span></p>
                </div>
                ...
                cards={[
                  {
                    ...
                    body: (
                      <div className="space-y-3 text-sm text-gray-500">
                        <p><span className="font-semibold text-gray-800">Tiêu đề gốc:</span> {config.sourceTitle || 'Không có'}</p>
                        <p><span className="font-semibold text-gray-800">URL:</span> {config.sourceUrl}</p>
                        <div>
                          <p className="mb-1 font-semibold text-gray-800">Heading đã dùng</p>
                          <pre className="whitespace-pre-wrap font-mono text-xs">{config.extractedHeadings || 'Không có heading'}</pre>
                        </div>
                      </div>
                    ),
                  },
                  {
                    ...
                    body: (
                      <div className="space-y-2 text-sm text-gray-500">
                        ...
                        <button
                          type="button"
                          onClick={() => handleEditorChange(`${editorHtml}<p>Nguồn tham khảo: <a href="${config.sourceUrl}" target="_blank" rel="nofollow noopener">${config.sourceTitle || config.keyword}</a></p>`)}
                          className="rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100"
                        >
                          Chèn link nguồn vào cuối bài
                        </button>
                      </div>
                    ),
                  },
...
function DEFAULT_DB_CONFIG(article: DbArticlePayload): UrlRewriteConfig {
  return {
    sourceUrl: article.keyword,
    extractedHeadings: '',
    extractedContent: '',
    sourceTitle: article.selectedTitle || article.keyword,
