...
interface DbArticlePayload {
  ...
  targetLength: number;
  aiProvider: string;
  brandConfig?: NewsRewriteConfig['brandConfig'];
  outline?: {
...
function buildPublishReadiness(input: {
  ...
  const items: PublishSignal[] = [
    ...
    { key: 'length', label: 'Độ dài đạt ngưỡng tối thiểu', pass: wordCount >= 500, detail: `${wordCount.toLocaleString()}/500 từ`, priority: 'high' },
...
export default function VietLaiTinTucGeneratePage() {
  ...
  const [config, setConfig] = useState<NewsRewriteConfig | null>(null);
  ...
  const [result, setResult] = useState<NewsRewriteResult | null>(null);
  ...
  const aiCheckStorageKey = useMemo(
    () => (runId ? `aicheck:rewrite-news:${runId}` : undefined),
    [runId],
  );
  ...
  useEffect(() => {
    document.title = 'Generate Viết Lại Tin Tức - Content Agent';
    void bootstrap();
  ...
  async function bootstrap() {
    const storedConfig = sessionStorage.getItem('vltt_config');
    const storedArticleId = sessionStorage.getItem('vltt_article_id');
    const storedRunId = sessionStorage.getItem('vltt_run_id');
    const storedOriginalWc = sessionStorage.getItem('vltt_original_wc');
    const storedResult = sessionStorage.getItem('vltt_result');

    if (runIdParam || !storedConfig || !storedArticleId || !storedRunId) {
      const targetRunId = runIdParam || storedRunId;
      if (!targetRunId) {
        router.replace('/viet-lai-tin-tuc');
        return;
      }
    ...
    try {
      const nextConfig = JSON.parse(storedConfig) as NewsRewriteConfig;
      ...
      if (storedResult) {
        const parsedResult = JSON.parse(storedResult) as NewsRewriteResult;
  ...
  async function loadFromDatabase(targetRunId: string) {
    ...
    try {
      ...
      const nextConfig: NewsRewriteConfig = article.outline?.config ?? DEFAULT_DB_CONFIG(article);

      sessionStorage.setItem('vltt_config', JSON.stringify(nextConfig));
      sessionStorage.setItem('vltt_run_id', article.runId);
      sessionStorage.setItem('vltt_article_id', article.id);

      setConfig(nextConfig);
      setArticleId(article.id);
      setRunId(article.runId);
      setOriginalWordCount(article.targetLength || 0);
      writeSessionAICheckState(article.runId ? `aicheck:rewrite-news:${article.runId}` : undefined, article.outline?.aiCheck);

      if (article.htmlContent) {
        const restoredResult: NewsRewriteResult = {
          runId: article.runId,
        ...
        sessionStorage.setItem('vltt_result', JSON.stringify(restoredResult));
        setLoading(false);
        return;
      }
  ...
  async function startGeneration(currentConfig: NewsRewriteConfig, currentRunId: string, currentArticleId: string) {
    setLoading(true);
    setError('');
    setStreamText('');
    setResult(null);
    setEditorHtml('');
    setEditTitle(currentConfig.keyword || currentConfig.originalTitle || 'Viết lại tin tức');
    ...
    try {
      const response = await fetch('/api/viet-lai-tin-tuc/stream', {
        method: 'POST',
      ...
      while (true) {
        ...
        for (const event of events) {
          ...
          if (payload.type === 'done' && payload.data) {
            ...
            sessionStorage.setItem('vltt_result', JSON.stringify(payload.data));
            setLoading(false);
          }
        }
      }
  ...
  async function saveDraft(createVersion: boolean) {
    ...
    try {
      const response = await fetch(`/api/articles/${articleId}/save`, {
        ...
        body: JSON.stringify({
          ...
          outline: {
            ...
            stage: 'generate',
            aiCheck: readSessionAICheckState(runId ? `aicheck:rewrite-news:${runId}` : undefined),
            config,
          },
      ...
      const updated: NewsRewriteResult = {
        ...result,
        html: editorHtml,
        title: editTitle,
        metaDescription: editMetaDescription,
      ...
      setResult(updated);
      sessionStorage.setItem('vltt_result', JSON.stringify(updated));
    ...
    return (
      <div className="h-full flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-800">Đang viết lại tin tức...</h2>
          <p className="text-sm text-gray-500 mt-1 max-w-md">
            AI đang giữ facts cốt lõi và tạo một bản tin mới dễ đọc hơn.
          </p>
          {streamText && (
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            type="button"
            onClick={() => router.push('/viet-lai-tin-tuc')}
  ...
  return (
    <div className="h-full bg-gray-50 flex flex-col">
      <div className="px-6 py-4 bg-white border-b border-gray-200 flex items-center justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs text-blue-600 font-semibold uppercase tracking-wide">Viết Lại Tin Tức</p>
        ...
        <div className="flex-1 min-w-0 grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div className="border-r border-gray-200 bg-white min-h-0 flex flex-col">
            <div className="px-4 py-3 border-b border-gray-200">
              <h2 className="text-sm font-semibold text-gray-800">Tin gốc</h2>
              <p className="text-xs text-gray-500 mt-1">Style: {config.style}</p>
            </div>
          ...
          <div className="bg-white min-h-0 flex flex-col">
            <div ref={editorShellRef} className="flex-1 min-h-0">
              <RichArticleEditor
                ...
                onNewArticle={() => router.push('/viet-lai-tin-tuc')}
              />
            </div>
          </div>
        </div>
                ...
                <GenerateQualityPanel
                  ...
                  summaryItems={[
                    ...
                    },
                    {
                      ...
                      tone: currentWordCount >= 500 ? 'good' : 'warn',
                    },
                  ]}
                >
                  <div className="mt-4">
                    ...
                    body: (
                      <div className="space-y-3 text-sm text-gray-500">
                        ...
                        <p className="whitespace-pre-wrap text-xs leading-relaxed">
                          {config.originalHtml.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 4000) || 'Không có nội dung gốc.'}
                        </p>
                      </div>
                    ),
                  },
...
function DEFAULT_DB_CONFIG(article: DbArticlePayload): NewsRewriteConfig {
  return {
    originalHtml: '',
