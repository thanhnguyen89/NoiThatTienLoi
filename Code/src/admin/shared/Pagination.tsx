    ...
    <nav className="cate-paging" aria-label="Phân trang">
      <ul className="cate-paging__list">
        ...
        <li className={`cate-paging__item ${currentPage <= 1 ? 'cate-paging__item--disabled' : ''}`}>
          {currentPage > 1 ? (
            <Link href={buildHref(currentPage - 1)} className="cate-paging__link cate-paging__prev" aria-label="Trang trước">
              ‹
            </Link>
          ) : (
        ...
        {pages.map((p, i) =>
          p === '...' ? (
            ...
            <li key={p} className={`cate-paging__item ${p === currentPage ? 'is-active' : ''}`}>
              <Link href={buildHref(p as number)} className="cate-paging__link">
                {p}
              </Link>
            </li>
          )
        ...
        <li className={`cate-paging__item ${currentPage >= totalPages ? 'cate-paging__item--disabled' : ''}`}>
          {currentPage < totalPages ? (
            <Link href={buildHref(currentPage + 1)} className="cate-paging__link cate-paging__next" aria-label="Trang sau">
              ›
            </Link>
          ) : (
